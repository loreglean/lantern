...
bitmap_painter::bitmap_painter(unsigned int width, unsigned int height)
...
void bitmap_painter::clear(unsigned char bytes_value)
{
	memset(m_data, bytes_value, m_data_total_size);
}

void bitmap_painter::draw_pixel(unsigned int x, unsigned int y, unsigned char r, unsigned char g, unsigned char b)
{
	unsigned int pixel_first_byte_index = m_pitch * y + x * 4;
