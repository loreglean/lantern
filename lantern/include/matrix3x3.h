...
namespace lantern
{
	...
	*/
	...
	public:
		...
		*/
		matrix3x3f(float const array[3][3]);
		...
		matrix3x3f operator*(matrix3x3f const& m) const;
		...
		matrix3x3f inversed_precalc_det(float const det) const;
		...
		*/
		static matrix3x3f scale(float const x, float const y, float const z);

		/** Generates uniform scale matrix
		* @param s Scale along each axis
		*/
		static matrix3x3f uniform_scale(float const s);

		/** Generates rotation around x-axis
		* @param radians Radians to rotate for
		*/
		static matrix3x3f rotation_around_x_axis(float const radians);

		/** Generates rotation around y-axis
		* @param radians Radians to rotate for
		*/
		static matrix3x3f rotation_around_y_axis(float const radians);

		/** Generates rotation around z-axis
		* @param radians Radians to rotate for
		*/
		static matrix3x3f rotation_around_z_axis(float const radians);

		/** Generates rotation around specified axis
		* @param axis Axis to rotate around
		* @param radians Radians to rotate for
		*/
		static matrix3x3f rotation_around_axis(vector3f const& axis, float const radians);

		static const matrix3x3f IDENTITY;
	};

	template<typename T>
	vector3<T> operator*(vector3<T> const& v, matrix3x3f const& m)
	{
		return vector3<T>{
