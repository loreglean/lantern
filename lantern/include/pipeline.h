...
namespace lantern
{
	...
	private:
		...
		*/
		...
		/** Saves edges coefficients for homogeneous algorithm 
		* @param binds List of binds
		* @param coefficients_storage Storage for edge coefficients
		* @param index0 First triangle vertex index in a mesh
		* @param index1 Second triangle vertex index in a mesh
		* @param index2 Third triangle vertex index in a mesh
		* @param vertices_matrix_inversed Inversed triangle matrix
		*/
		template<typename TAttr>
		...
		/** Sets bind points values using edge coefficients and w value
		@param binds List of binds
		@param coefficients_storage Storage for edge coefficients
		@param point Pixel to set value at
		@param w W value
		*/
		template<typename TAttr>
	...
	void pipeline::draw(mesh const& mesh, TShader& shader, texture& target_texture)
	{
		...
		bind_attributes(shader.get_vector3f_bind_points(), mesh.get_vector3f_attributes(), m_binded_vector3f_attributes);
	...
	inline bool pipeline::is_point_on_positive_halfspace_top_left(
		float const edge_equation_value, float const edge_equation_a, float const edge_equation_b) const
	{
